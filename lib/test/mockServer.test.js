...
describe('Query generation', () => {
    ...
    it('Generates multiple queries2', () => {
        expect(queries.map((q) => q.originalQuery)).toEqual(2);
    });
    it('Generates multiple reports', () => {
        expect(mutations.map((q) => q.originalQuery)).toEqual(2);
    });
    it('Generates multiple mutations', () => {
        expect(mutations.map((q) => q.name).filter((q) => q.match(/createPlaylist/g)).length).toEqual(2);
    });
    it('Generates multiple mutations 2', () => {
        expect(mutations.map((q) => q.name)).toEqual(2);
    });
