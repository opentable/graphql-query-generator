...
function getExamplesFrom(comment) {
    ...
    const result = [];
    let matches = null;
    const test = new RegExp(/(?<alias>[\w]*)?\s*:?\s*(?<name>[\w]*)\s*(?<args>\([^)]*\))\s*(?<directive>@[\w]*\([^)]*\))*/g);
    while ((matches = test.exec(examplesDescription)) && matches.length > 1) {
        ...
        const query = `${groups.alias || ''}${groups.alias ? ':' : ''}${groups.name}${groups.args}${groups.directive ? ' ' + groups.directive : ''}`;
        result.push(query);
    }
    return result;
}
