...
module.exports = {
  getAllFields: function getAllFields(field, typeDictionary, visitedFields, isRoot = true, parentFieldTypeDefinition = null) {
    let coverageMagic = [];
    ...
    _.forIn(allFields, (childField) => {
      ...
      if (visitedFields.indexOf(getSkipKey(childFieldType, childField, fieldTypeDefinition)) === -1) {
        coverageMagic.push(`${fieldTypeName}___${childFieldTypeName}___${childField.name}`);
        const subWalk = getAllFields(childField, typeDictionary, visitedFields, false, fieldTypeDefinition);
  ...
  getQuerableFields: function getQuerableFields(field, typeDictionary, visitedFields, isRoot = true, parentFieldTypeDefinition = null) {
    let coverageMagic = [];
    const fieldTypeName = magiclyExtractFieldTypeName(field);
    const fieldTypeDefinition = typeDictionary[fieldTypeName];
    if(isRoot) {
    ...
    _.forIn(allFields, (childField) => {
      ...
      if (
        noOfPossibleQueries > 0 &&
        visitedFields.indexOf(getSkipKey(childFieldType, childField, fieldTypeDefinition)) === -1
      ) {
        coverageMagic.push(`${fieldTypeName}___${childFieldTypeName}___${childField.name}`);
        const subWalk = getQuerableFields(childField, typeDictionary, visitedFields, false, fieldTypeDefinition);
        coverageMagic = coverageMagic.concat(subWalk);
      }
    });
