var examplesSection = new RegExp(/Example[s]?:/);
exports["default"] = {
    /**
     * @example
    ...
    getExamplesFrom: function getExamplesFrom(comment) {
        ...
        var what = comment.split(examplesSection);
        if (what.length !== 2)
            return [];
        var examplesDescription = what[1];
        var result = [];
        var matches = null = null;
        var test = new RegExp(/(?<alias>[\w]*)?\s*:?\s*(?<name>[\w]*)\s*(?<args>\([^)]*\))\s*(?<directive>@[\w]*\([^)]*\))*/g);
        // Forgive me
        while ((matches = test.exec(examplesDescription)) && matches.length > 1) {
            ...
            var query = "" + (groups.alias || '') + (groups.alias ? ':' : '') + groups.name + groups.args + (groups.directive ? ' ' + groups.directive : '');
            result.push(query);
        }
        return result;
    },
     ...
     *  exports.default.shouldFollow('Examples:country(\ncId: 1\n)+NOFOLLOW') // => true
     */
    shouldFollow: function (description) {
        if (!description) {
            return true;
        }
